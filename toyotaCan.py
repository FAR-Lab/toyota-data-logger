...
if __name__ == "__main__":
    ...
    # start up the CAN logger
    os.system('sudo ip link set can0 type can bitrate 500000 listen-only on && sudo ip link set can0 up')

    bus = can.interface.Bus(results.channel, bustype=results.interface, can_filters=can_filters)
    print('Can Logger (Started on {})\n'.format(datetime.datetime.now()))
    notifier = can.Notifier(bus, [can.Printer(results.log_file), can.Printer()])
